...
class Extended_Viterbi:
    ...
    def compute_forward_potials(self):
        ...
        for i in xrange(n_positions - 1):
            ...
            self.pos_forward_potials.append(np.sum(current_path_fwd ,axis = 1))
    ...
    def compute_backward_potials(self):
        ...
        self.pos_backward_potials.append(np.ones(len(self.pos_potials[-1]),
                                                 dtype = np.double))
        for i in xrange(n_positions - 1)[::-1]:
    ...
    def compute_potials(self):
        n_positions = len(self.pos_potials)
        self.compute_forward_probas()
        self.compute_backward_probas()
        self.pos_potials.append(self.pos_forward_potials[0] * 
                                self.pos_backward_potials[-1])
        for i in xrange(n_positions - 1):
            self.pos_potials.append(self.pos_forward_potials[i] * 
                                    self.pos_backward_potials[-i])
            self.path_potials.append(self.path_forward_potials[i] *
    ...
    def normalize_potials(self):
        ...
        self.pos_potials[0]             /= np.sum(self.pos_potials[0])
        for i in xrange(n_positions - 1):
            ...
            self.pos_potials[i]             /= np.sum(self.pos_potials[i])
            #
